 ...
 *
 * @notice Oracle contract that fetches price data from Chainlink Oracle.
 *
 ...
 */
contract ChainlinkOracle is IChainlinkOracle, Initializable, Ownable2StepUpgradeable {
    // -- State variables --
     ...
     */
    address public override chainlinkOracle;
     ...
     * Chainlink protocol to update the price on-chain.
     */
    uint256 public override ageValidityBuffer;

    /**
     * @notice Allowed decimals of the price.
     */
    uint256 public constant ALLOWED_DECIMALS = 18;
     ...
     *
     * @dev The function attempts to retrieve the price from the Chainlink oracle.
    ...
    function peek(
        ...
        try AggregatorV3Interface(chainlinkOracle).latestRoundData() returns (
            uint80, int256 answer, uint256, uint256 updatedAt, uint80
        ) {
            // Ensure the fetched price is greater than zero
            if (answer <= 0) revert InvalidPrice();

            // Ensure the price is not outdated
            uint256 minAllowedAge = block.timestamp - (ageValidityPeriod + ageValidityBuffer);
            if (updatedAt < minAllowedAge) revert OutdatedPrice({ minAllowedAge: minAllowedAge, actualAge: updatedAt });

            // Ensure the price has the correct decimals
            uint256 decimals = AggregatorV3Interface(chainlinkOracle).decimals();
            if (decimals == 0 || decimals > 18) revert InvalidDecimals();

            // Set success flag and return the price
            success = true;
            rate = uint256(answer) * 10 ** (ALLOWED_DECIMALS - decimals);
        } catch {
