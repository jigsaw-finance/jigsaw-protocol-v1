...
interface IChainlinkOracle is IOracle {
    ...
    event ChainlinkOracleCreated(
        address indexed underlying, address indexed chainlinkOracle, uint256 ageValidityPeriod
    );
    ...
    /**
     * @notice Thrown when Chainlink oracle returns invalid price.
     * @dev Zero and negative prices are not valid for the standard token price feeds.
     */
    error InvalidPrice();

    /**
     * @notice Thrown when the Chainlink oracle's price feed has an invalid number of decimals.
     * @dev The number of decimals must be greater than 0 and less than or equal to 18 to be considered valid.
     */
     ...
     * @return The address of the Chainlink Oracle.
     */
